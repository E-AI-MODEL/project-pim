...
export function modelGateFor(
  ...
  const action = isAction(actionOrLegacyProfile)
    ? actionOrLegacyProfile
    : (settingsOrAction as Action);
  const settingsInput = isAction(actionOrLegacyProfile) ? settingsOrAction : undefined;
  const settings = coerceDetectionSettings(
    settingsInput as DetectionLayerSettings | string | undefined,
  );
  ...
  if (!ner) {
    return isLocal
      ...
      : {
          ...
          detail: "BERT is gekozen voor deze controle, maar nog niet geladen of geverifieerd.",
        };
  }
  ...
  if (ner.status === "verified")
    return {
      verified: true,
      reason: "bert-required-and-verified",
      detail: "BERT-integriteit is geverifieerd.",
    };
  if (ner.status === "mismatch")
    return {
      verified: false,
      reason: "bert-required-but-mismatch",
      detail: "BERT-integriteit klopt niet. Uitgaande acties blijven geblokkeerd.",
    };
  ...
  if (ner.status === "placeholder") {
    return isLocal
      ...
      : {
          verified: false,
          reason: "bert-required-but-placeholder",
          detail: "BERT is niet productie-verified voor uitgaande actie.",
        };
  }
  return isLocal
    ? {
