...
const RUBRIC_PATTERNS: { test: (s: PrivacySignals) => boolean; apply: (t: string) => string }[] = [
  {
    ...
    test: (s) => s.directPii.some((p) => p.category === "context_small_group") ||
                 s.contextualPii.some((p) => p.category === "context_small_group"),
    apply: (t) =>
      t.replace(/\b(?:groep|klas)\s?[1-8][A-Za-z]?\b/gi, "een groep")
  ...
  },
  {
    ...
    test: (s) => s.directPii.some((p) => p.category === "context_care" || p.category === "context_incident") ||
                 s.contextualPii.some((p) => p.category === "context_care" || p.category === "context_incident"),
  ...
  },
  {
    ...
    test: () => true,
    apply: (t) =>
      t.replace(/\b(?:gisteren|vandaag|vorige week|afgelopen \w+)\b/gi, "in een recente periode")
       .replace(/\b\d{1,2}\s?(?:januari|februari|maart|april|mei|juni|juli|augustus|september|oktober|november|december)\b/gi, "een periode"),
  },
  {
