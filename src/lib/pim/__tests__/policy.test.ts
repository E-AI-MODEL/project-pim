...
describe("policy.decide — basis", () => {
  it("ALLOW bij anonymous + display + pass + verified + certified payload", () => {
    const d = decide({
      mode: "anonymous", action: "display", signals: emptySignals, draftCheck: passCheck,
  ...
  it("BLOCK bij modelVerified=false", () => {
    const d = decide({
      mode: "anonymous", action: "copy", signals: emptySignals, draftCheck: passCheck,
      modelVerified: false, profileId: "education-nl-full", payloadType: "draft_anonymous_certified",
    });
