...
function Cell({ v }: { v: string | boolean }) {
  ...
  if (v === false)
    return (
      <span className="inline-flex items-center gap-1.5 text-red text-sm font-medium">
        <X className="h-4 w-4" /> nee
      </span>
    );
  ...
  return (
    <>
      <PageHero
        ...
        title={
          <>
            Anoniem <span className="text-muted-foreground">of</span>{" "}
            <span className="text-cyan">pseudoniem</span>
          </>
        }
          ...
          <table className="w-full min-w-[640px]">
            <thead>
              <tr className="border-b border-border/60">
                ...
                <th className="text-left p-4 font-mono text-[11px] uppercase tracking-wider text-purple">
                  Anonymous
                </th>
                <th className="text-left p-4 font-mono text-[11px] uppercase tracking-wider text-cyan">
                  Pseudonymous
                </th>
              </tr>
            </thead>
...
function Con({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex items-start gap-2">
      <X className="h-4 w-4 text-red mt-0.5 flex-shrink-0" />
      <span>{children}</span>
    </div>
  );
}
