...
const ROLES = [
  ...
  { icon: FileCheck, name: "Minimal Audit", role: "Legt alleen besluitmetadata vast — nooit inhoud.", decides: false, color: "green" },
];
  ...
  return (
    <>
      <PageHero
        ...
        title={<>Strikte <span className="text-primary">rolverdeling</span></>}
      ...
      <section className="mx-auto max-w-7xl px-6 py-14">
        <div className="text-xs font-mono uppercase tracking-wider text-cyan mb-2">01 · Wie mag beslissen?</div>
        <h2 className="font-display text-3xl font-bold mb-8">Eén beslislaag, één bron van waarheid.</h2>
        <div className="overflow-x-auto panel">
          <table className="w-full min-w-[640px]">
            <thead>
              <tr className="border-b border-border/60">
                <th className="text-left p-4 font-mono text-[11px] uppercase tracking-wider text-muted-foreground">Laag</th>
                <th className="text-left p-4 font-mono text-[11px] uppercase tracking-wider text-muted-foreground">Rol</th>
                <th className="text-right p-4 font-mono text-[11px] uppercase tracking-wider text-muted-foreground">Mag finale toestemming geven?</th>
              </tr>
            </thead>
            <tbody>
              {ROLES.map((r) => {
                const Icon = r.icon;
                return (
                  <tr key={r.name} className="border-b border-border/30 last:border-0 hover:bg-accent/20 transition-colors">
                    <td className="p-4">
                      <div className="flex items-center gap-3">
                        <div className={`h-9 w-9 grid place-items-center rounded-lg bg-${r.color}/10 text-${r.color}`}>
                    ...
                    <td className="p-4 text-right">
                      {r.decides ? (
                        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-green/15 text-green text-xs font-semibold">JA</span>
                      ) : (
                        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-card border border-border/60 text-muted-foreground text-xs">nee</span>
                      )}
                    </td>
                  </tr>
                );
      ...
      <section className="mx-auto max-w-7xl px-6 py-14">
        <div className="text-xs font-mono uppercase tracking-wider text-orange mb-2">02 · Datastromen</div>
        <h2 className="font-display text-3xl font-bold mb-10">Van raw input tot egress.</h2>
        <div className="grid md:grid-cols-3 gap-4">
          {[
            { num: "01", title: "Detection", body: "Code regex + browser SLM produceren spans en confidence. Niets verlaat de browser.", color: "cyan" },
            { num: "02", title: "Processing", body: "Anonymize of pseudonymize. Mapping (indien aanwezig) gaat in een AES-GCM container.", color: "purple" },
            { num: "03", title: "Decision + Egress", body: "PIM beslist per actie. Egress Guard handhaaft. Audit logt enkel metadata.", color: "orange" },
          ].map((s) => (
      ...
      <section className="mx-auto max-w-7xl px-6 py-14">
        <div className="panel-glow p-8 md:p-10">
          <div className="text-xs font-mono uppercase tracking-wider text-primary mb-2">03 · Fail-closed regels</div>
          <h2 className="font-display text-2xl md:text-3xl font-bold mb-6">Wanneer PIM altijd blokkeert</h2>
