...
const SOURCES = [
  {
    name: "EDPB Guidelines 01/2025 on Pseudonymisation",
...
function CompliancePage() {
  ...
  const checks: { ok: boolean; label: string; detail: string }[] = [
    {
      ok: true,
      label: "Validated education-nl profile",
      detail: `${RELEASE_1_PROFILES.length} profielen vrijgegeven: ${RELEASE_1_PROFILES.join(", ")}`,
    },
    {
      ok: true,
      label: "Baseline detectoren actief",
      detail: "rules + special lexicon verplicht in elk release-profiel",
    },
    {
      ok: integrity.every((r) => r.status === "verified"),
    ...
    {
      ok: true,
      label: "Pseudonymous egress invariants",
      detail: "send_external_ai / export / copy / print / share = altijd BLOCK",
    },
    {
      ok: true,
      label: "Draft Check Guard invariants",
      detail: "Residuele directe PII = fail-closed; mode-mix = fail-closed",
    },
    {
      ok: true,
      label: "PIM default block",
      detail: "Onbekende verdict-paden vallen terug op BLOCK",
    },
    {
      ok: true,
    ...
    {
      ok: true,
      label: "Geen claim van 100% anonimiteit",
      detail: "Expliciet zichtbaar in UI en compliance-pagina",
    },
  ];
      ...
      <section className="mx-auto max-w-5xl px-6 py-14 space-y-10">
        <Block title="Productiegate — v3-2" tone={allGreen ? "green" : "red"}>
            ...
            {checks.map((c) => (
              <li key={c.label} className="flex items-start gap-2.5">
                ...
                <div className="min-w-0">
                  <div className={`font-semibold ${c.ok ? "text-foreground" : "text-red"}`}>
                    {c.label}
                  </div>
                ...
                {integrity.map((rec) => (
                  <li key={rec.key} className="flex flex-wrap gap-2">
                    ...
                    <span className="text-muted-foreground">
                      expected:{" "}
                      {rec.expected.startsWith("PLACEHOLDER:")
                        ? rec.expected
                        : rec.expected.slice(0, 16) + "…"}
                    </span>
                  </li>
                ))}
              </ul>
          ...
          <div className="mt-4 font-mono text-[10px] text-muted-foreground">
            catalog entries: {Object.values(MODEL_CATALOG).length} · release-1 profielen:{" "}
            {RELEASE_1_PROFILES.join(", ")}
          </div>
        </Block>

        <Block title="Wat we claimen" tone="green">
          <p>
            Project PIM dwingt een technische scheiding af tussen ruwe input, pseudonieme
            verwerking, anonieme verwerking, mapping, lokale opslag, netwerkverkeer en export.
          </p>
          <ul className="mt-3 space-y-1.5 text-sm">
            <li>
              • In anonieme modus bestaat geen mapping en kan de tool originelen niet terugzetten.
            </li>
            <li>
            ...
            <li>
              • Modellen leveren signalen of herschrijvingen, maar nemen nooit het eindbesluit.
            </li>
        ...
        <Block title="AI Act — toegestane technische focus" tone="green">
          <ul className="flex flex-wrap gap-2">
            {ALLOWED.map((x) => (
              <li
                key={x}
                className="font-mono text-xs px-2.5 py-1 rounded-full bg-green/10 text-green border border-green/30"
              >
                {x}
              </li>
            ))}
