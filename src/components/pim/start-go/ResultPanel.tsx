...
import { computeSignals, draftCheck, decide, DEFAULT_DETECTION_SETTINGS, type DetectionLayerSettings, type PiiCategory, type Action } from "@/lib/pim";
...
export function ResultPanel({
  ...
  onOriginalChange, detectionSettings = DEFAULT_DETECTION_SETTINGS, disabledCategories, thresholdOverrides,
}: Props) {
  const directSpans = signals.directPii;
  const allSpans = [...signals.directPii, ...signals.contextualPii];
  const [tab, setTab] = useState<"original" | "safe">(decision.verdict === "BLOCK" ? "original" : "safe");
    ...
    <section className="space-y-4 animate-fade-in">
      ...
      {decision.verdict === "BLOCK" && directSpans.length > 0 && (
        <div className="rounded-xl border border-red-500/30 bg-red-500/5 p-4 space-y-2">
          ...
          <div className="text-xs text-muted-foreground pt-1 leading-relaxed"><span className="text-foreground font-medium">Volgende stap:</span> verwijder deze gegevens, of zet de modus op <span className="text-foreground font-medium">Anoniem</span> — PiM voert de controle dan automatisch opnieuw uit.</div>
        </div>
      )}
      <div className="space-y-2">
        <div className="flex items-center justify-between gap-2 flex-wrap">
          <div className="inline-flex rounded-lg border border-border/50 bg-card/40 p-0.5">
            <button type="button" onClick={() => setTab("original")} className={`px-3 py-1.5 text-xs font-medium rounded-md transition-colors ${tab === "original" ? "bg-primary/20 text-foreground" : "text-muted-foreground hover:text-foreground"}`}>Origineel met markeringen{allSpans.length > 0 && <span className="ml-1.5 text-[10px] font-plex-mono text-muted-foreground">{allSpans.length}</span>}</button>
            <button type="button" onClick={() => setTab("safe")} disabled={decision.verdict === "BLOCK"} className={`px-3 py-1.5 text-xs font-medium rounded-md transition-colors ${tab === "safe" ? "bg-primary/20 text-foreground" : "text-muted-foreground hover:text-foreground"} disabled:opacity-40 disabled:cursor-not-allowed`}>Veilige versie</button>
          </div>
          <div className="text-[10px] text-muted-foreground">{tab === "safe" ? (isEdited ? `Handmatig bewerkt — live opnieuw beoordeeld: ${liveSafeVerdict ?? "—"}` : "Bewerkbaar — wijzigingen worden direct opnieuw gecontroleerd") : "Bewerkbaar — markeringen verversen mee tijdens typen"}</div>
        </div>
        {tab === "original" ? (
          <div className="space-y-2">
            {onOriginalChange && <textarea value={originalText} onChange={(e) => onOriginalChange(e.target.value)} className="w-full min-h-[120px] rounded-xl border border-border/50 bg-background/60 p-3 text-sm outline-none focus:ring-2 focus:ring-primary/30" />}
