...
export function StartGoShell({ compact = false }: { compact?: boolean } = {}) {
  ...
  }, [text, mode, action, integrity, profileId, thresholdOverrides, disabledCategories, modelSpans]);
  ...
  return (
    <div className={compact ? "space-y-5" : "mx-auto max-w-3xl px-4 sm:px-6 py-8 sm:py-14 space-y-7"}>
      <InputPanel
        text={text}
        onTextChange={(v) => { setText(v); setResult(null); setEgressMsg(null); setLlmMsg(null); }}
        onStart={run}
        onExample={onExample}
        busy={busy}
        compact={compact}
...
function LocalModelStrip({
  ...
  const nerPct = normalizePct(nerStatus?.progress?.pct);
  const llmPct = normalizePct(llmStatus?.progress?.pct);
...
function ModelCard({
  icon, title, kind, pct, sub, detail, buttonLabel, onClick, disabled,
}: {
  icon: "ner" | "llm";
  title: string;
  ...
  return (
    <div className={`rounded-xl border p-3 min-w-0 ${tone}`}>
      ...
      {detail && (
        <div className="mt-2 text-[10px] font-plex-mono text-[#e8edf3]/55 break-words">{detail}</div>
      )}
    </div>
  );
}
