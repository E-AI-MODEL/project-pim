  ...
  return (
    ...
    >
      ...
      >
        ...
        <ChevronDown className={`h-5 w-5 shrink-0 text-slate-300/75 transition-transform ${open ? "rotate-180" : ""}`} />
      </button>
          ...
          <div className="rounded-2xl border border-white/8 bg-[#07182a]/70 p-4">
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-xs text-slate-300/72">Alleen voor debug. Uit betekent: PiM zoekt die categorie niet meer.</p>
              </div>
              <button type="button" onClick={() => setShowTechnical((v) => !v)} className="rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2 text-xs font-medium text-slate-200 hover:bg-white/[0.06]">
            ...
            {showTechnical && (
              <div className="mt-4 space-y-4">
                ...
                {offCount > 0 && (
                  <button type="button" onClick={onResetCategories} className="inline-flex items-center gap-1.5 rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2 text-xs text-slate-200 hover:bg-white/[0.06]">
                ...
                {overriddenCount > 0 && (
                  <button type="button" onClick={onResetThresholds} className="inline-flex items-center gap-1.5 rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2 text-xs text-slate-200 hover:bg-white/[0.06]">
  ...
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-1 gap-3 lg:grid-cols-3">
        <LayerCard title={DETECTION_LAYER_COPY.regex.title} description={DETECTION_LAYER_COPY.regex.description} status="Altijd aan" />
      ...
      <div className="rounded-2xl border border-white/8 bg-[#07182a]/70 p-4">
        <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
          <div className="min-w-0">
            ...
            <p className="mt-1 text-xs leading-relaxed text-slate-300/75">Lokaal taalmodel dat woorden labelt als persoon, organisatie of plaats. Regex, Lexicon en Context blijven aan.</p>
          </div>
          {settings.bert !== "off" && ner && (
            <button type="button" onClick={ner.onStart} disabled={ner.status?.loading || ner.status?.working} className="inline-flex h-10 shrink-0 items-center justify-center rounded-xl border border-cyan-300/25 bg-cyan-300/10 px-4 text-sm font-semibold text-cyan-100 hover:bg-cyan-300/16 disabled:cursor-not-allowed disabled:opacity-50">
              {ner.status?.loading ? "Laden en testen…" : ner.status?.working ? "BERT werkt" : "Laden en testen"}
            </button>
          )}
        </div>

        <div className="mt-4 grid grid-cols-1 gap-2 sm:grid-cols-3">
          <BertOption value="off" selected={settings.bert === "off"} title="Uit" text="Alleen Regex, Lexicon en Context." onClick={selectBert} />
          <BertOption value="100mb" selected={settings.bert === "100mb"} title="100 MB" text="Sneller en lichter." onClick={selectBert} />
          <BertOption value="180mb" selected={settings.bert === "180mb"} title="180 MB" text="Groter en zwaarder." onClick={selectBert} />
        </div>
...
function BertOption({ value, selected, title, text, onClick }: { value: BertChoice; selected: boolean; title: string; text: string; onClick: (value: BertChoice) => void }) {
  return (
    <button
      type="button"
  ...
  return (
    <div className="rounded-2xl border border-white/8 bg-[#07182a]/70 p-4 space-y-4">
      <div>
        ...
        <p className="mt-1 text-xs text-slate-300/72">Kies per soort gegeven: automatisch wissen of alleen markeren.</p>
      </div>
      <div className="grid grid-cols-1 gap-3 lg:grid-cols-2">
        {CATEGORY_SECTIONS.map((section) => (
          <div key={section.label} className="rounded-2xl border border-white/8 bg-white/[0.025] p-3">
            <div className="mb-2 px-1 text-[10px] font-semibold uppercase tracking-[0.18em] text-slate-400">{section.label}</div>
            <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-2">
              {section.cats.map((cat) => {
                const scrub = writer.autoRedact.has(cat);
                return (
                  <div key={cat} className="flex items-center justify-between gap-2 rounded-xl border border-white/8 bg-[#07182a]/55 px-2.5 py-2">
                    <span className="block truncate text-xs font-medium text-slate-100">{CATEGORY_LABELS[cat] ?? cat}</span>
                    <div className="inline-flex rounded-lg border border-white/10 bg-black/12 p-0.5 gap-0.5">
                      <button type="button" onClick={() => writer.onAutoRedactChange(cat, true)} className={`rounded-md px-2 py-1 text-[10px] font-semibold ${scrub ? "bg-emerald-300/18 text-emerald-100" : "text-slate-400 hover:text-slate-100"}`}>Wis</button>
                      <button type="button" onClick={() => writer.onAutoRedactChange(cat, false)} className={`rounded-md px-2 py-1 text-[10px] font-semibold ${!scrub ? "bg-amber-300/18 text-amber-100" : "text-slate-400 hover:text-slate-100"}`}>Markeer</button>
                    </div>
                  </div>
                );
              })}
      ...
      <button type="button" onClick={() => writer.onStrictChange(!writer.strict)} className={`rounded-2xl border px-3 py-2.5 text-left transition-colors ${writer.strict ? "border-amber-300/45 bg-amber-300/10" : "border-white/8 bg-white/[0.025] hover:bg-white/[0.055]"}`}>
        <div className="flex items-center justify-between gap-3">
          <span className="text-xs font-semibold text-slate-100">Strenge cijfercontrole</span>
          <span className={`text-[10px] font-semibold uppercase tracking-[0.18em] ${writer.strict ? "text-amber-100" : "text-slate-400"}`}>{writer.strict ? "aan" : "uit"}</span>
        </div>
      </button>
    </div>
  );
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="rounded-full border border-white/10 bg-white/[0.035] px-2 py-0.5 text-[11px] text-slate-300">{status}</span>
          </div>
        ...
        <div key={section.label} className="rounded-2xl border border-white/8 bg-white/[0.025] p-3">
          ...
          <div className="space-y-1.5">
            {section.cats.map((cat) => {
              ...
              return (
                <label key={cat} className={`flex cursor-pointer items-center justify-between gap-2 rounded-xl border px-2.5 py-2 transition-colors ${off ? "border-rose-400/25 bg-rose-400/8" : "border-white/8 bg-[#07182a]/50 hover:bg-white/[0.055]"}`}>
                  <span className={`block truncate text-xs font-medium ${off ? "text-rose-100" : "text-slate-100"}`}>{CATEGORY_LABELS[cat] ?? cat}</span>
...
function IntegrityList({ integrity }: { integrity: ModelIntegrityRecord[] }) {
  if (integrity.length === 0) return <p className="text-xs text-slate-400">Nog geen modelchecks.</p>;
  return (
    <ul className="space-y-1.5">
      {integrity.map((r) => (
        <li key={r.key} className="flex justify-between gap-2 rounded-xl border border-white/8 bg-white/[0.025] px-3 py-2 font-plex-mono text-[11px]">
...
function Chip({ children }: { children: React.ReactNode }) {
  return <span className="rounded-full border border-white/8 bg-black/14 px-2 py-0.5 shadow-sm">{children}</span>;
}
...
function modelStatus(status: NerStatus | null | undefined, bert: BertChoice): { label: string; className: string } {
  if (bert === "off") return { label: "Uit", className: "border-white/10 bg-white/[0.035] text-slate-300" };
  if (status?.working) return { label: "Werkt", className: "border-emerald-300/35 bg-emerald-300/12 text-emerald-100" };
  if (status?.loading) return { label: "Laden/testen", className: "border-amber-300/35 bg-amber-300/12 text-amber-100" };
  if (status?.error || status?.healthError) return { label: "Fout", className: "border-red-300/35 bg-red-300/12 text-red-100" };
  return { label: `${bert === "100mb" ? "100 MB" : "180 MB"} niet geladen`, className: "border-white/10 bg-white/[0.035] text-slate-300" };
}
