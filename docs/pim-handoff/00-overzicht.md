...
- **Lock-principe** in plaats van silent blocks:
  - *Niet-strikt (default)*: alles mag. Externe AI en export met BERT uit geven `ALLOW_WITH_WARNING` ("mogelijk niet alles gedetecteerd"). Kopiëren, printen, sharen mogen altijd.
  - *Strikt (gebruiker zet zelf aan)*: externe AI en export worden geblokkeerd totdat BERT aan staat. Knoppen krijgen slot-icoon.
